...
def generate(
    ...
    # R1 JSON / CLI edge cases may pass str; generation requires int (e.g. gen_length % block_length).
    gen_length = int(gen_length)
    block_length = int(block_length)
    steps = int(steps)

    # Auto-detect mask_id from tokenizer or model config if not provided
    if mask_id is None:
    ...
    for num_block in tqdm(range(num_blocks), disable=(dist.get_rank() != 0)):
        ...
        for i in range(steps_per_block):
            ...
            del logits, logits_with_noise, x0, x0_p, confidence

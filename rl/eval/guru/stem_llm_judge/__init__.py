"""
Optional external LLM judge (OpenAI-compatible /v1/chat/completions).

Guru training routes ``stem_web*`` through local ``naive_dapo`` in
``rl.eval.guru.default_compute_score`` (no vLLM/OpenAI). Import and call
``compute_score`` here only if you want LLM-as-judge outside that path.

Prerequisite when using this module: ``STEM_LLM_JUDGE_URL``.
"""
...
_STEM_JUDGE_URL_WARNED: list[bool] = [False]
...
def compute_score(data_source: str,
    ...
    try:
        ...
        if not _STEM_JUDGE_URL_WARNED[0]:
            # run_multi_train.py disables warnings.warn — must print for visibility.
            print(
                "[R1][stem_llm_judge] "
                f"{e!r}. Set STEM_LLM_JUDGE_URL (OpenAI-compatible …/v1/chat/completions) "
                "for stem_web* Guru rows; else reward=0 and WD1 loss may stay 0. "
                "(This message prints once.)",
                file=sys.stderr,
                flush=True,
            )
            _STEM_JUDGE_URL_WARNED[0] = True
        return 0.0

"""
This code follows wd1 official implementation.
"""
...
class RevDiffuGRPOTrainer(GRPOTrainer):
    """
    Our objective NSR + PSR for main rl method
    ...
    def _forward_with_mask(self, model, input_ids, block_length=None):
        """Call model forward. For LLaDA2-MoE the inner-model patch adds the
        block-causal mask automatically when ``attention_mask is None``."""
        if is_llada2_moe(model):
            # Let the class-level patch create the mask (same path as eval).
    ...
    def compute_loss(
        ...
        # Positive Sample Reweighting (PSR) loss computation
        ce_loss = -((per_token_logps[0] * completion_mask).sum(-1) * is_weights).sum()
        # Negative Sample Reweighting (NSR) loss computation
        n_is_weights = torch.softmax(-advantages, dim=0)
        min_likeli_loss = (
            (per_token_logps[0] * completion_mask).sum(-1) * n_is_weights
        ).sum()
        return ce_loss + min_likeli_loss
    ...
    def generate(
        ...
        For LLaDA2-MoE models, uses the official block-aligned incremental
        approach (pad to block boundary, process one block at a time, forward
        only up to current_window_end). This avoids the misalignment between
        attention-mask blocks and generation blocks that caused garbage output.
        """
    ...
    def get_logits(self, model, batch, prompt_index, cfg_scale, mask_id):
        ...
        input = batch
        logits = self._forward_with_mask(model, input).logits
        # Apply Dream logits shift if necessary
        logits = apply_dream_logits_shift(logits, model)
    ...
    def _generate_and_score_completions(
        ...
        # Use max_length padding for DDP: ensures identical sequence length across ranks,
        # avoiding gradient shape mismatch (RuntimeError: Detected mismatch between collectives)
        max_len = self.max_prompt_length or 512
        ...
        with unwrap_model_for_generation(
            ...
            for i in range(0, prompt_ids.size(0), generation_batch_size):
                ...
                # Enable dynamic_generate for both "b1_wll" and "r1_b1_wll".
                if "b1_wll" in str(self.args.trainer_type):
        ...
        # Reward weights are all one by default
        ...
        # GRPO advantages: (r - group_mean) / (group_std + eps); zero when std ~ 0.
        # Aligns with TRL GRPO and stabilizes multi-domain batches vs raw centered rewards.
        ...
        for i, reward_func in enumerate(self.reward_funcs):
            ...
            # Skip multi-domain router: mirrors scalar reward before extras; train/reward is authoritative.
        ...
        if (
            ...
            if self.accelerator.is_main_process:
                if is_rich_available():
                    ...
                    # HPC envs often lack rich → no table otherwise; Guru math needs \boxed.
                    print(
                        f"[log_completions step={self.state.global_step}] "
                        f"(pip install rich for full table; first 3 rows)",
                        flush=True,
                    )
                    for li in range(min(3, len(completions_to_log))):
                        c = completions_to_log[li]
                        text = (
                            c[0]["content"]
                        ...
                        rk = (
                            rewards_to_log[li]
                            if li < len(rewards_to_log)
                            else None
                        )
                        has_box = "\\boxed" in text
                        print(
                            f"  #{li} reward={rk} has_boxed={has_box} "

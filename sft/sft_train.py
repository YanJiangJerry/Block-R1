...
def parse_args():
    ...
    parser.add_argument(
        "--debugging",
        action="store_true",
        help="Use while debugging model - only disables wandb logging",
    )
...
def train_model(args, tokenizer, model):
    ...
    # Auto-detect mask_token_id from tokenizer
    mask_token_id = (
    ...
    trainer = dLLMTrainer(
        ...
        data_collator=dLLMDataCollator(
            tokenizer=tokenizer, mask_token_id=mask_token_id, max_length=args.max_length
        ),

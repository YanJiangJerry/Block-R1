#!/bin/bash
# Block-R1 + D1 (DiffuGRPO) — this repo's R1 multi-domain stack uses ``r1_d1`` for
# standard diffusion GRPO; there is no separate ``r1_grpo`` trainer name.

#SBATCH --job-name=block_r1_grpo
#SBATCH --nodes=1
...
export WANDB_PROJECT=block_r1_grpo

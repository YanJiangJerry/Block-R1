#!/bin/bash
# Block-R1: multi-domain offline JSONL + StableDRL (per-sample br1_best_block_size).
# Prereq: build train.jsonl via block_r1.sh (or equivalent build_block_r1).

#SBATCH --job-name=block_r1_stable_drl
#SBATCH --nodes=1
...
export WANDB_PROJECT="${WANDB_PROJECT:-block_r1_stable_drl}"
...
RUN_NAME="${RUN_NAME:-block_r1_stable_drl_multi}"
NUM_ITER=12
